...
background = PhotoImage(file = "136920.png")
...
Button(root, text = "upload",image = upload,
             command = open_img).place(x=400, y=300)


Label(root, text="\n                    OUTPUT\n",
           width=25,
           font=("Bauhaus 93", 20)).place(x=800, y=600)
Label(root, text="",textvariable = output,
                    font=("Bauhaus 93", 20)).place(x=800, y=600)


detect = PhotoImage(file = r"detect1.png")
Button(root, text="detect",image = detect,
                            command = callback).place(x=400,y=610)
